...
class coreFerjeruta {
	 ...
	 * @constructor
	 */
	StartAutoRefresh(subname, interval) {
		...
		if (this.AutoRefreshData[subname].ProcId == 0) {
			this.AutoRefreshData[subname].ProcId = setInterval(function () {
				_fr.AutoRefreshTimer(subname, interval);
			}, interval);
		} else {
		}
	 ...
	 * @constructor
	 */
	AutoRefreshTimer(subname, timeoutval) {
	 ...
	 * @constructor
	 */
	Initialize(refreshWhenDone, onCompleteFunction) {
		var pobj = this;
	 ...
	 * @constructor
	 */
	GetNext(ferryline, departurepoint, dayofweek, hour, minute) {
	 ...
	 * @constructor
	 */
	RefreshNotifications(oncomplete) {
		var pobj = this;
			...
			.each(function (l) {
				...
				if (pobj.GetSetting('ShowTimeOfArrival') == true) {
					...
					if (tript != undefined && tript != "") {
						...
						var d = spacetime('January 6 2020', 'Europe/Oslo'); // Generic Monday
						d = d.time(departure.TimeOfDay);
						d = d.add(tript, 'minutes');
						...
						var arrivaltime = d.unixFmt('HH:mm');
